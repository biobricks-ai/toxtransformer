...
def get_mie_predictions(predictions, prompt, top_n=20):
    ...
    # Take max similarity across all three channels (MIE title, AO title, AOP title)
    # Previously used inner merges which required all three to score >0.2 simultaneously
    prompt_mie = pd.concat([prompt_mie, prompt_ao, prompt_aop])
