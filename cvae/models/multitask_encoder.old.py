...
class FastMultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cos: Optional[torch.Tensor] = None, 
        ...
        if self.use_flash:
            attn_mask = None
            if mask is not None:
                # mask is [B, T, T] - full attention mask
            ...
            if mask is not None:
                # mask is [B, T, T], expand to [B, 1, T, T] for heads
...
class MultitaskEncoder(nn.Module):
    ...
    def forward(self, selfies: torch.Tensor, properties: torch.Tensor, 
        ...
        """
        Ultra-fast forward pass optimized for compilation.
        """
        ...
        # Construct attention mask: [B, T, T]
        # Layout: [SELFIES tokens | prop0, val0, prop1, val1, ...]
        #
        # Attention rules:
        # - SELFIES tokens: can see all SELFIES (bidirectional), CANNOT see any PV tokens
        # - Property tokens: can see all SELFIES + previous PV pairs
        # - Value tokens: can see all SELFIES + previous PV pairs + current property query
        B, T = full_sequence.shape[:2]
        selfies_len = selfies.shape[1]
        
        # Create the structural attention mask
        attn_mask = torch.ones(T, T, dtype=torch.bool, device=full_sequence.device)
        
        # Block SELFIES from seeing PV tokens (upper-right block)
        if selfies_len > 0 and T > selfies_len:
            attn_mask[:selfies_len, selfies_len:] = False
        
        # Apply causal mask to PV region (lower-right block)
        pv_len = T - selfies_len
        if pv_len > 0:
            causal_mask = torch.tril(torch.ones(pv_len, pv_len, dtype=torch.bool, device=full_sequence.device))
            attn_mask[selfies_len:, selfies_len:] = causal_mask
        
        # Combine structural mask with padding/dropout mask
        key_mask = full_mask.unsqueeze(1)  # [B, 1, T] - can attend TO these positions
        query_mask = full_mask.unsqueeze(2)  # [B, T, 1] - can attend FROM these positions
        attn_mask = attn_mask.unsqueeze(0) & key_mask & query_mask  # [B, T, T]
        
        # Decode with fast transformer
        decoded = self.decoder(full_sequence, mask=attn_mask)
        
        # Extract predictions at property positions (where we want to predict values)
        decoded_values = decoded[:, selfies_len::2]  # Start at first property position, take every 2nd
